...
static void printOnScreen(int value, int row, int col){
	if(value > 3) {
		oled_putString(col, row,  (uint8_t*)"-", OLED_COLOR_WHITE, OLED_COLOR_BLACK);
	}
	else{
		oled_putString(col, row,  (uint8_t*)"*", OLED_COLOR_WHITE, OLED_COLOR_BLACK);
	}
    ...
    while (1) {
        ...
        if (btn1 == 0) {
        	...
        	if(print){
        		...
        		oled_putString(1, 49, buf, OLED_COLOR_WHITE, OLED_COLOR_BLACK);

        		LPC_TMR16B0->TCR = 2;
				printOnScreen(hold, row, col);
